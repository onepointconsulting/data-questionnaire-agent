...
from data_questionnaire_agent.test.provider.consultant_provider import (
    create_simple_consultant, create_consultant_rating
)
...
if __name__ == "__main__":
    ...
    async def test_save_session_consultants():
        ...
        counter = await save_session_consultant_ratings(dummy_session_id, consultant_ratings)
        assert counter == 1
        read_crs = await read_session_consultant_ratings(dummy_session_id)
        assert len(read_crs.consultant_ratings) == len(consultant_ratings.consultant_ratings)
