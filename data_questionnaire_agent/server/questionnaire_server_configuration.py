...
from data_questionnaire_agent.model.global_configuration import GlobalConfigurationProperty, GlobalConfiguration
from data_questionnaire_agent.service.persistence_service_async import update_global_configuration
from data_questionnaire_agent.server.server_support import CORS_HEADERS, routes, handle_error
...
async def update_global_configuration_web(request: web.Request) -> web.Response:
    async def process(request: web.Request):
        ...
        match json_content:
            case {"message_lower_limit": message_lower_limit, "message_upper_limit": message_upper_limit}:
                properties = [
                    GlobalConfigurationProperty(config_key='MESSAGE_LOWER_LIMIT', config_value=str(message_lower_limit)),
                    GlobalConfigurationProperty(config_key='MESSAGE_UPPER_LIMIT', config_value=str(message_upper_limit))
                ]
