...
if __name__ == "__main__":
    ...
    def deal_with_unrelated_questions():
        questionnaire: Questionnaire = create_questionnaire_initial_question("Which is the capital of India")
        has_questions_chain: LLMChain = sentiment_chain_factory()
        clarification_agent: AgentExecutor = create_clarification_agent()
        asyncio.run(process_clarifications_chainlit(questionnaire, 1, has_questions_chain, clarification_agent, False))

    def deal_with_related_questions():
        questionnaire: Questionnaire = create_questionnaire_initial_question("What do you mean by data quality?")
        has_questions_chain: LLMChain = sentiment_chain_factory()
        clarification_agent: AgentExecutor = create_clarification_agent()
        asyncio.run(process_clarifications_chainlit(questionnaire, 1, has_questions_chain, clarification_agent, False))
