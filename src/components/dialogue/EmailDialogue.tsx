...
export  default function EmailDialogue() {
  ...
  const emailUrl = useMemo(() => `${reportUrl}/email/${sessionId}`, [reportUrl, sessionId]);
  ...
  async function onOk() {
    ...
    try {
      ...
      setFeedbackMessage('Error sending email. Fetch failed. Please try again later.');
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <dialog
      data-model={true}
      id={EMAIL_DIALOGUE_ID}
      className="email-dialogue"
    >
      ...
      {sending && <Spinner/>}
      {feedbackMessage && <Alert feedback={feedbackMessage} onClose={() => setFeedbackMessage('')}/>}
      <ButtonPanel onOk={onOk} onClose={onClose} okText="Send email" disabled={disabled()} />
    </dialog>
  )
}